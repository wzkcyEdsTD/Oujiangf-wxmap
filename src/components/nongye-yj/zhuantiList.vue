...
<template>
  <div class="t-meizhou">
    ...
    <!-- 筛选列表 -->
    <div class="shaixuan" v-if="searchMap.url!='http://services.wzmap.gov.cn/server/rest/services/TDT/POI/MapServer/0'">
    ...
    <div class="tmap-search-result" style="height:calc(100% - 128px);overflow:auto;">
      <template v-for="(i,idx) in searchMap.page.data">
        <template v-if="searchMap.url=='http://services.wzmap.gov.cn/server/rest/services/TDT/POI/MapServer/0'">
        ...
        <div class="s-result_item" v-else>
          ...
          <div class='s_result_item-pic'>
            <img v-if="i.attributes['Name']" :src="i.attributes['ServerUrl']+'/'+(i.attributes['SmallLocation'] || i.attributes['Location'])+'/'+i.attributes['Name']" alt="">
  ...
  export default {
    ...
    methods:{
      ...
      async searchInfo(obj,loadmap){
        ...
        if(!obj.attributes){
          ...
          var querytask = new QueryTask({
          	url:"http://services.wzmap.gov.cn/server/rest/services/TDT/POI/MapServer/0"
          });
          var query = new Query();
        ...
        if(!loadmap){
          if(obj.attributes.ADDRESS){
            ...
            Util.ajax.post('/pubApi/wzmap/poiimg?guid='+obj.attributes.FEATUREGUID).then(function (res) {
              ...
              self.$router.push({name:"nongyeMap",query:{guid:obj.attributes.FEATUREGUID,loadmap:loadmap?'true':'false'}});
            });
          }else{
      ...
      async searchpoi(flush,url,tag,list,iszhuanti,iszhoubian){
        ...
        var layerurl = "http://services.wzmap.gov.cn/server/rest/services/TDT/POI/MapServer/0";
        var layerurl2 = "http://services.wzmap.gov.cn/server/rest/services/TDT/POI/MapServer/0";
        var searchIdx = 6;
        var sname = '';
        if(url){
          layerurl = url;
        ...
        var query = new Query();
        query.returnGeometry = true;
        query.outFields = nongyeConfig['outFields'+(searchIdx+1)];//FEATUREGUID,CENTERX,CENTERY,NAME,ADDRESS,TAG
        ...
        if(tag){
          //面点
          if(!/.*?(4|8)$/.test(layerurl)){
            query.where = " tdt.DBO."+sname+".TAG LIKE '%"+tag+"%' AND tdt.DBO."+sname+".NAME is not null ";
          }else{
          ...
          if(self.searchMap.areaName){
            query.where+=" AND tdt.DBO."+sname+".AREA='"+self.searchMap.areaName+"' "
          }
          if(self.searchMap.monthName){
            query.where+=" AND tdt.DBO."+sname+".MONTH like '%"+self.searchMap.monthName+"%' "
          }
          //tdt.DBO."+sname+".
          ...
          if(!sfound){
            self.$set(self.searchMap.shaixuan,'option',[]);
          	...
          	self.searchMap.tag = '';
			if(self.searchMap.areaName){
				query.where+=" AND tdt.DBO.农业旅游.AREA='"+self.searchMap.areaName+"' "
			}
        }
        //同组
        if(list){
