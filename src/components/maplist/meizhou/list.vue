<style lang="less" scoped>
  ...
  /*.t-meizhou{
    .t-item{
      display: flex;border-bottom:5px solid #f8f8f8;padding:8px 10px;
      .t-item_info{
        flex:1;
        text-indent:10px;
        .t-item_info1{color:#666;border-left:5px solid @theme-color;margin:5px 0;}
        .t-item_info2{text-indent:15px;color:#aaa;}
      }
      .t-item_img{
        img{width:66px;display: block;margin:5px 0;}
      }
    }
  }*/
</style>
  ...
  <div class="t-meizhou">
    ...
    <div class="header2_right" style="position:fixed;top:0;z-index:1;">
      <!--<div class="header_search">
        <div class="man">
          <input type="text" v-model="page.key" class="text" @keyup.enter="searchkey" placeholder="请输入关键字查询">
    ...
    <div class="shaixuan" style="position: fixed;width:100%;top:34px;z-index: 1;background: #f3f3f3;">
      ...
      <div :class="page.bean.area?'on':''" @click="areaShow=true">
    ...
    <!--<div class="t-item" v-for="(i,idx) in datas">
      <div class="t-item_info" @click="detail(i)">
        <div class="t-item_info1">{{i.title}}</div>
        <div class="t-item_info2">{{i.summary}}</div>
      </div>
      <div class="t-item_img">
        <img src="@/images/m-i1.png" alt="" @click="detail(i)">
        <img src="@/images/m-i2.png" alt="" @click="ditu(i)">
      </div>
    </div>-->
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      ditu(i){
        this.$router.push({name:'mzmap',query:{mid:i.id,title:i.title,mlevel:i.mlevel,lng:i.lng,lat:i.lat}});
      },
      clickArea(key,item){
        if(item.label=='全部'){
          this.areaVal = '';
