  ...
  <div class="newidx">
    ...
    <tab>
      <template v-if="catalogList">
        <tab-item v-for="(i,idx) in catalogList" :selected="group['show'+(idx+1)]" @on-item-click="groupShow(i.id)">{{i.name}}</tab-item>
      </template>
    </tab>
    <div class="news-list" style="margin-top:15px;">
      <div class="news-item" style="display: flex;background: #fff;padding:8px;border-bottom: 1px solid #ddd" v-for="i in noticeList" @click="toNewsCon(i)">
        <div v-if="i.pic"><img :src="i.pic" alt="" width="90"></div>
        <div v-else><img  src="@/images/nopic.jpg" width="90"></div>
